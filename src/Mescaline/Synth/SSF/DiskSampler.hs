...
-- runVoice voice = 
--     switch (constant voice &&& switchEvt voice) $ \(Voice _ buf) -> Voice (-1) buf
--     where
--         switchEvt voice = proc () -> do
--             e1 <- delayUntil (t + dur) -< Event voice
--             e2 <- send (stopVoice voice params (t + dur) -< e1
--             e3 <- waitFor (n_end nid) -< e2
--             returnA -< e3
            -- tu' <- utcr
            -- liftIO $ putStrLn ("node end: " ++ show voice)
    -- e4 <- freeVoice cache voice
    -- return ()
